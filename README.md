...
1. 每个阶段固定为 12 周。
2. 每月按 4 个教学周规划，因此每个三个月阶段共 12 周。
3. 第 12 周同时承担阶段项目、验收、总结和下一阶段准备，不另设第 13 周。
...
3. 每日课程必须在“目标”之前设置“今日新名词”章节；当天首次出现的新名词分别用一句话解释，已经解释过的名词除非需要深化，否则不重复解释。
4. 所有研究明确数据口径、时间对齐、复权方式与交易成本。
5. 每周最后一天完成测试、复盘与 Git 提交。
6. 数学公式使用标准 Markdown：行内 `$R_t$`，独立公式使用 `$$...$$`。
7. 每日课程文件直接保存在对应周目录，不再创建 `days/` 中间目录。
8. 每个阶段第 12 周必须完成阶段项目、验收清单和阶段总结。
